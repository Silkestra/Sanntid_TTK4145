...
Elevator             elevator;
ElevOutputDevice     outputDevice;

func __attribute__ ((constructor))
void __attribute__((constructor)) fsm_init(){
...
void fsm_onDoorTimeout(void){
    ...
    switch(elevator
						...
						void fsm_onFloorArrival(int newFloor){
							...
							case EB_Moving:
								if(requests_shouldStop(elevator)){
									...
									timer_start(elevator.config.doorOpenDuration_s);
									setAllLights(elevator);
									elevator.behaviour = EB_DoorOpen;rection(D_Stop);
						outputDevice.doorLight(1);
						elevator = requests_clearAtCurrentFloor(elevator);
						timer_start(elevator.config.doorOpenDuration_s);
						setAllLights(elevator);
		...
		(elevator.config.doorOpenDuration_s);
            elevator = requests_clearAtCurrentFloor(elevator);
            setAllLights(elevator);
            break;
        case EB_Moving:
